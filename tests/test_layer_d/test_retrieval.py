...
# ---------------------------------------------------------------------------
# page_image_refs propagation into RankedResult (修正 ⑦)
# ---------------------------------------------------------------------------

def _make_scored_point_with_image_refs(
    chunk_id: str = "chunk-img",
    page_image_refs: dict = None,
) -> MagicMock:
    """Scored point whose payload includes page_image_refs."""
    payload = {
        "chunk_id": chunk_id,
        ...
        "has_handwriting": False,
        "embedding_text": "test",
        "page_image_refs": page_image_refs if page_image_refs is not None else {},
    }
    pt = MagicMock()
    pt.id = _chunk_id_to_point_id(chunk_id)
    pt.score = 0.5
...
class TestPageImageRefsInRankedResult:
    def test_page_image_refs_populated_from_payload(self):
        refs = {"1": "images/page_1.png", "2": "images/page_2.png"}
        pt = _make_scored_point_with_image_refs(page_image_refs=refs)
        retriever, _ = _make_retriever([pt])

        results = retriever.search(FAKE_DENSE, FAKE_SPARSE)

        assert len(results) == 1
        assert results[0].page_image_refs == refs

    def test_page_image_refs_empty_dict_when_absent(self):
        pt = _make_scored_point("chunk-no-refs")  # no page_image_refs in payload
        retriever, _ = _make_retriever([pt])

        results = retriever.search(FAKE_DENSE, FAKE_SPARSE)

        assert results[0].page_image_refs == {}

    def test_page_image_refs_in_parent_aggregated_chunk(self):
        refs = {"3": "images/page_3.png"}
        row_pt = _make_scored_point(
            "unit-img_row0", chunk_type="row", parent_chunk_id="unit-img", score=0.5
        )
        parent_payload = {
            "chunk_id": "unit-img",
            "chunk_type": "table",
            "parent_chunk_id": None,
            "retrieval_unit_id": "unit-img",
            "retrieval_weight": 0.95,
            "source_tool": "azure_cu",
            "source_pages": [3],
            "display_markdown": "table content",
            "page_image_refs": refs,
        }
        parent_pt = MagicMock()
        parent_pt.id = _chunk_id_to_point_id("unit-img")
        parent_pt.payload = parent_payload
        ...
        parent_result = next(r for r in results if r.chunk_id == "unit-img")
        assert parent_result.page_image_refs == refs

    def test_ranked_result_default_page_image_refs_is_empty_dict(self):
        r = RankedResult(
            chunk_id="x",
            chunk_type="paragraph",
            parent_chunk_id=None,
